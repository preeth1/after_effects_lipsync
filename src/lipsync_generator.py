...
class LipsyncGenerator:
    ...
    def generate_timestamp_script_list(self):
        ...
        self.convert_times_to_frame_rates(script_data, frame_rate)
    ...
    @staticmethod
    def convert_times_to_frame_rates(script_data, frame_rate):
        one_frame_seconds = 1 / frame_rate
        for script_data_entry in script_data:
            script_data_entry["start_frame"] = int(script_data_entry["start_time_s"] / one_frame_seconds)
            script_data_entry["end_frame"] = int(script_data_entry["end_time_s"] / one_frame_seconds)

    @staticmethod
    def create_output_dictionary(script_data):
        # Set the first frame to always start out neutral
        output_phoneme_dictionary = collections.OrderedDict({-1: 'neutral'})
